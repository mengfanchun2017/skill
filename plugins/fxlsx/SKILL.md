...
> **⚠️ 数字必须写入为数值类型，不能是字符串。** `cell.value = 753192`（int）+ `cell.number_format = '#,##0'` → 显示 753,192。**禁止** `cell.value = '753,192'`（字符串），否则 Excel 当作文本，无法参与公式计算，排序/筛选失效，且千分位显示不可控。无论 openpyxl 还是 OfficeCLI，写入采购量/金额/百分比时一律写数值 + 设 format。
