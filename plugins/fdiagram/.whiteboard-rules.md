...
4. **Write it into Feishu as an editable whiteboard**, then **look at the real board too:**
   `npx -y @larksuite/whiteboard-cli@^0.2.11 -i <dir>/diagram.svg --to openapi | npx -y @larksuite/whiteboard-cli@^0.2.11 whiteboard +update --whiteboard-token <tok> --source - --input_format openapi --idempotent-token <unique> --overwrite`
   then `npx -y @larksuite/whiteboard-cli@^0.2.11 whiteboard +query --whiteboard-token <tok> --output_as image --output <dir>`
...
npx -y @larksuite/whiteboard-cli@^0.2.11 docs +create --api-version v2 \
  --content '<title>My board</title><whiteboard type="blank"></whiteboard>'
```
Then write the SVG to that `block_token` with the `whiteboard +update` command in step 4. To add a
board to an existing doc the user gave, fetch it (`npx -y @larksuite/whiteboard-cli@^0.2.11 docs +fetch`) or append a
