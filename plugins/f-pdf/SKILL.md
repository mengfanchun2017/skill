...
user-invocable: true
description: |
  PDF 内容提取 + 翻译原语 — 文字/图片/表格/元数据提取，PDF 翻译文档生成。
  独立使用或委托 f-feishu 创建飞书文档。
...
### 内容提取
1. **文字提取**：工作流 A（PyMuPDF，含标题层级/粗斜体/列表/表格检测）
2. **图片提取**：工作流 B（PyMuPDF + 纯 Python zlib 回退）
3. **图片验证**：`minimax understand_image` 逐张过滤 logo/装饰图

### 创建飞书文档：委托 f-feishu skill
文档创建和格式规范由 f-feishu 编排（`docs +create` + lark-table XML + colgroup=822 等格式约束）。
...
| 翻译 | Claude LLM |
| 文档创建 | f-feishu skill（委托文档创建 + 格式规范） |
