...
用户意图
  ...
  ├─ "画架构图"/"流程图"/"时序图"          → 委托 f-diagram skill
  ├─ "生成PPT"/"做slides"                 → 委托 f-pptx skill
...
- 架构图/流程图/时序图等由 f-diagram 生成，通过 `block_insert_after` 插入
- 图表在对应内容位置嵌入，不在末尾
...
3. 图表：委托 f-diagram skill，通过 `block_insert_after` 嵌入
...
> 架构/流程图委托 f-diagram。本工作流用于**数据/分析图**（matplotlib/plotly）：每个图建独立子文档。
...
| 生成 PPT | f-pptx | 委托 f-pptx skill |
| 画图表 | f-diagram | 委托 f-diagram skill |
| 数据图（图子文档） | f-feishu 本工作流 G | lark-cli + python |
| 文件上传 | lark-drive | `lark-cli drive +upload` |
| PDF 提取+翻译 | f-pdf | 委托 f-pdf skill |

---

## 用户配置

用户说"配置 f-feishu"时，读取 `config.yaml` 展示可配置项，用 AskUserQuestion 让用户修改，写回文件。
...
> f-feishu 创建/编辑的文档。每次操作后追加。格式：`[标题](url) | 日期 | 说明`
...
> **注意**: f-logme 管辖的文档（OKR/SUM/Worklog）索引在 `skills/f-logme/SKILL.md` 的「线上文档索引」节。f-feishu 索引只记录 f-feishu 直接创建的文档（研究/合并等）。
