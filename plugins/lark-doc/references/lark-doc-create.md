...
- **返回 URL 域名陷阱**：`+create` 返回的 `url` 字段硬编码 `www.feishu.cn` 域名（如 `https://www.feishu.cn/docx/XXX`），但实际可访问的域名是 `my.feishu.cn`。创建文档后必须将 URL 中的 `www` 替换为 `my`，或通过 `drive +search` 查询正确的 `my.feishu.cn` URL。
- **`--parent-token` 传 Wiki 节点 vs Drive 文件夹**：`--parent-token` 接受 Wiki 知识库节点 token（`entity_type: WIKI`），创建后文档归属该 Wiki。但 `drive +move` 需要的是 Drive 文件夹 token（`entity_type: DOC, doc_types: FOLDER`），两者不同。移动已有文档到 Wiki 下需先找到对应的 Drive 文件夹 token。
